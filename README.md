...
This compiles the client into a jar located at `./target/product-sdk-1.0.0.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'product-api:product-sdk:1.0.0'
}
```
...
This will compile the client and package all dependencies into a jar located at `./target/product-sdk-1.0.0.jar`.
...
```java
DynamoDBProductSdkClientBuilder builder = DynamoDBProductSdk.builder();
```
...
```java
DynamoDBProductSdk client = DynamoDBProductSdk.builder()
...
```java
DynamoDBProductSdk client = DynamoDBProductSdk.builder().build();
client.shutdown();
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`DynamoDBProductSdk`) is created for all actions (resource + method) in your API.
...
```java
DynamoDBProductSdk client = DynamoDBProductSdk.builder().build();
PostApiRootResult result = client.postApiRoot(new PostApiRootRequest());
```
...
```java
DynamoDBProductSdk client = DynamoDBProductSdk.builder().build();
client.postApiRoot(new PostApiRootRequest().sdkRequestConfig(
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of DynamoDBProductSdkException.
```java
try {
    client.postApiRoot(...);
} catch(DynamoDBProductSdkException e) {
   // All service exceptions will extend from DynamoDBProductSdkException.
   // Any unknown or unmodeled service exceptions will be represented as a DynamoDBProductSdkException.
